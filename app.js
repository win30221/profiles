...
const ICONS = {
  ...
  power:'<path d="M12 2v10"/><path d="M7.1 4.8a9 9 0 1 0 9.8 0"/>',
...
let activeApp = '', selectedProject = 0, selectedYear = '2019';
...
const copy=()=>window.HUGO_CONTENT[window.HugoLocale.language];
const isZh=()=>window.HugoLocale.language==='zh-TW';
...
function renderAbout(){const c=copy().about,m=copy().metrics;return title(c.kicker,c.title,c.intro)+`<div class="stats">${m.map(([value,unit,label])=>`<div><strong>${value.replace('+','')}<span>+</span></strong><span>${label}</span></div>`).join('')}</div><div class="about-detail-grid"><div><span class="detail-label">01 / ENGINEERING</span><h3>${c.engineeringTitle}</h3><p>${c.engineering}</p></div><div><span class="detail-label">02 / LEADERSHIP</span><h3>${c.leadershipTitle}</h3><p>${c.leadership}</p></div></div><button class="text-button" data-app="experience">${c.journey} <span>↗</span></button>`;}
function renderExperience(){const c=copy().experience,years=c.years,item=years[selectedYear]||years[Object.keys(years).at(-1)];return title(c.kicker,c.title,c.intro)+`<p class="content-note">${c.note}</p><div class="timeline-tabs" role="tablist" aria-label="${isZh()?'工作年份':'Experience years'}">${Object.keys(years).map(year=>`<button role="tab" aria-selected="${year===selectedYear}" class="${year===selectedYear?'active':''}" data-year="${year}">${year}</button>`).join('')}</div><section class="timeline-detail" role="tabpanel"><span class="timeline-year">${item.company}</span><h3>${item.role}</h3><ul>${item.bullets.map(line=>`<li>${line}</li>`).join('')}</ul><span class="sample-label">${c.verified}</span></section>`;}
let projectFilter='all', projectDetail=false;
function renderProjects(){
  const c=copy().projects,projects=c.entries,p=projects[selectedProject]||projects[0];
  const kindLabel=project=>project.kind==='portfolio'?c.implemented:project.kind==='side'?c.side:c.real;
  if(projectDetail)return `<div class="file-toolbar"><button data-project-back>${icon('folder')} ${c.back}</button><span class="mono">hugo / projects / ${escapeHTML(p.short)}</span></div><article class="project-detail"><span class="detail-label">${kindLabel(p)}</span><h2>${p.title}</h2>${tags(p.stack)}<div class="case-grid">${c.labels.map((label,i)=>`<div><h3 class="detail-label">${label}</h3><p>${[p.problem,p.architecture,p.role,p.challenges,p.solution,p.result][i]}</p></div>`).join('')}</div></article>`;
  const rows=projects.map((project,i)=>({project,i})).filter(({project})=>projectFilter==='all'||(projectFilter==='backend'?project.kind!=='portfolio':project.kind==='portfolio'));
  return `<div class="file-toolbar"><span>${icon('folder')}</span><span class="mono">hugo <span class="muted">/</span> projects</span><span class="file-count">${rows.length} ${c.items}</span></div><div class="file-manager"><nav class="file-sidebar" aria-label="${isZh()?'專案分類':'Project categories'}">${[['all','folder',c.all],['backend','cpu',c.backend],['portfolio','globe',c.portfolio]].map(([id,ico,label])=>`<button data-project-filter="${id}" aria-pressed="${projectFilter===id}">${icon(ico)}${label}</button>`).join('')}</nav><div class="file-main"><div class="file-list" aria-label="${copy().ui.apps.projects}"><div class="file-columns" aria-hidden="true"><span>${c.name}</span><span>${c.type}</span></div>${rows.map(({project,i})=>`<button class="file-row" data-select-project="${i}" aria-pressed="${selectedProject===i}">${icon('folder')}<span>${project.short}</span><small>${kindLabel(project)}</small></button>`).join('')}</div><section class="file-preview" aria-label="${isZh()?'所選專案預覽':'Selected project preview'}"><span class="preview-folder">${icon('folder')}</span><div><span class="detail-label">${kindLabel(p)}</span><h2>${p.short}</h2>${tags(p.stack)}<p>${p.problem}</p><button class="os-button primary" data-open-project="${selectedProject}">${c.open} ${icon('arrow')}</button></div></section></div></div>`;
}
function renderSkills(){const c=copy().skills;return title(c.kicker,c.title,c.intro)+`<div class="skills-map">${c.groups.map(([name,items,description],i)=>`<section class="skill-cluster"><span class="detail-label">${name}<span>${String(i+1).padStart(2,'0')}</span></span>${tags(items)}<p>${description}</p></section>`).join('')}</div>`;}
function renderTerminal(){const zh=isZh();return `<div class="full-terminal"><div class="terminal-output" role="log" aria-label="${zh?'終端機輸出':'Terminal output'}"><p>HUGO OS [Version 1.0.0]<br>${zh?'Hugo Shell · 瀏覽檔案、應用程式與桌面設定。<br>輸入「help」開始探索。Tab 自動完成 · ↑ / ↓ 查看歷史紀錄。':'Hugo Shell · files, apps and desktop settings.<br>Type “help” to explore. Tab to complete · ↑ / ↓ for history.'}</p>${terminalHistory.map(line=>`<p class="${line.type==='command'?'command-echo':''}">${escapeHTML(line.text)}</p>`).join('')}</div><form id="terminal-form" class="terminal-input-line"><label for="terminal-command" class="prompt-user">visitor@hugo<span class="muted">:${escapeHTML(window.HugoOS?.cwd||'/')}$</span></label><input id="terminal-command" aria-label="${zh?'終端機指令':'Terminal command'}" autocomplete="off" autocapitalize="off" spellcheck="false"><button aria-label="${zh?'執行終端機指令':'Run terminal command'}">${icon('arrow')}</button></form><div class="terminal-completions">${['help','whoami','skills','projects','contact','clear'].map(cmd=>`<button data-command="${cmd}">${cmd}</button>`).join('')}</div></div>`;}
function renderResume(){const c=copy().resume;return title(c.kicker,c.title,c.intro)+`<div class="resume-actions"><a class="primary-button" href="assets/hugo-resume-current.pdf" download="Hugo-Resume.pdf">${c.download} ${icon('download')}</a><a href="assets/hugo-resume-current.pdf" target="_blank" rel="noopener">${c.preview} ↗</a></div><article class="resume-paper"><span class="eyebrow">${c.draft}</span><h3>HUGO / 蕭宏輝</h3><p>${c.role}</p><h4>${c.profileTitle}</h4><p>${c.profile}</p><div class="resume-metrics">${c.metrics.replaceAll('\n','<br>')}</div><h4>${c.technologyTitle}</h4><p>${c.technology}</p><h4>${c.leadershipTitle}</h4><p>${c.leadership}</p><h4>${c.experienceTitle}</h4><p>${c.experience}</p></article><p class="content-note">${c.note}</p>`;}
function safeUrl(url){try{const parsed=new URL(url);return ['https:','http:'].includes(parsed.protocol)?parsed.href:'';}catch{return '';}}
function renderContact(){const c=copy().contact;return title(c.kicker,c.title,c.intro)+`<div class="contact-links">${[['Email',PROFILE.email,'mail'],['GitHub',PROFILE.github,'globe'],['LinkedIn',PROFILE.linkedin,'network']].map(([label,value,ico])=>{const link=label==='Email'&&/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)?'mailto:'+encodeURIComponent(value):safeUrl(value);return link?`<a href="${escapeHTML(link)}" ${label!=='Email'?'target="_blank" rel="noopener noreferrer"':''}><span class="contact-label">${label}</span><span class="contact-value">${escapeHTML(value)}</span>${icon('arrow')}</a>`:`<div><span class="contact-label">${label}</span><span class="contact-value">${c.missing}</span>${icon(ico)}</div>`;}).join('')}</div><p class="content-note">${c.note}</p>`;}
const renderers={about:renderAbout,experience:renderExperience,projects:renderProjects,terminal:renderTerminal,skills:renderSkills,resume:renderResume,contact:renderContact};
const windows=new Map();
const APP_META={welcome:['Profile Overview','home'],about:['About Hugo','user'],experience:['Experience','clock'],projects:['Case Studies','folder'],terminal:['Terminal','terminal'],skills:['Skills','layers'],resume:['Resume PDF','file'],contact:['Contact','mail']};
const PINNED=['welcome','projects','resume'];
let windowOrder=0,desktopStarted=false,desktopStash=[];
function renderWelcome(){
 const all=copy(),c=all.welcome,years=Object.entries(all.experience.years),projects=all.projects.entries.filter(project=>project.kind==='real').slice(0,4);
 const sectionHead=(number,label,action='',app='')=>`<div class="profile-section-head"><span class="eyebrow">${number} / ${label}</span>${action?`<button data-app="${app}">${action} ${icon('arrow')}</button>`:''}</div>`;
 return `<article class="welcome-copy"><header class="profile-hero" aria-labelledby="profile-headline"><div><span class="eyebrow">${c.eyebrow}</span><h1 id="profile-headline">${c.headline}</h1><p class="welcome-role">${c.summary}</p><div class="welcome-actions"><button class="os-button primary" data-app="projects">${icon('folder')} ${c.primary}</button><button class="os-button" data-app="resume">${icon('file')} ${c.secondary}</button></div></div><aside class="profile-tenure" aria-label="${isZh()?'年資摘要':'Experience summary'}">${all.metrics.map(([value,unit,label])=>`<span><strong>${value}</strong><small>${unit}</small><em>${label}</em></span>`).join('')}</aside></header>
 <section class="profile-section profile-summary">${sectionHead('01',c.sectionSummary)}<div class="profile-summary-grid"><div><h2>${all.about.engineeringTitle}</h2><p>${all.about.engineering}</p></div><div><h2>${all.about.leadershipTitle}</h2><p>${all.about.leadership}</p></div></div></section>
 <section class="profile-section">${sectionHead('02',c.sectionCareer,c.allExperience,'experience')}<div class="profile-career">${years.map(([year,item])=>`<button data-year-launch="${year}"><time>${year}</time><span><strong>${item.role}</strong><small>${item.company}</small><em>${item.bullets[0]}</em></span>${icon('arrow')}</button>`).join('')}</div></section>
 <section class="profile-section">${sectionHead('03',c.sectionProjects,c.allProjects,'projects')}<div class="profile-projects">${projects.map((project,index)=>`<button data-project-launch="${index}"><span class="detail-label">${all.projects.real}</span><strong>${project.title}</strong><small>${project.result}</small><em>${project.stack.slice(0,4).join(' · ')}</em>${icon('arrow')}</button>`).join('')}</div></section>
 <section class="profile-section">${sectionHead('04',c.sectionSkills)}<div class="profile-skills">${all.skills.groups.map(([name,items])=>`<div><strong>${name}</strong><span>${items.join(' · ')}</span></div>`).join('')}</div></section>
 <section class="profile-section profile-credentials">${sectionHead('05',c.sectionCredentials)}<div><p><strong>${all.education.university}</strong><span>${all.education.award}</span><span>${all.education.training}</span></p><a href="mailto:${PROFILE.email}"><small>${c.contactLabel}</small>${PROFILE.email} ${icon('arrow')}</a></div></section>
 <footer class="welcome-foot"><span><i class="status-dot"></i>${c.footer}</span><button data-app="about">${c.about} ${icon('arrow')}</button></footer></article>`;
}
...
function syncDesktop(){
  ...
  $('#active-app-label').textContent=APP_META[activeApp]?.[0]||copy()?.ui.desktop||'Desktop';
  ...
  for(const id of ids){
    ...
    button.querySelector('.dock-label').textContent=APP_META[id][0];
    ...
    const status=running?(state.minimized?(isZh()?' — 已最小化':' — minimized'):(isZh()?' — 執行中':' — running')):'';
    button.setAttribute('aria-pressed',String(activeApp===id));button.setAttribute('aria-label',`${APP_META[id][0]}${status}`);button.title=button.getAttribute('aria-label');
  }
}
...
function createAppWindow(id){
  ...
  element.innerHTML=`<header class="os-titlebar"><button class="mobile-back" data-window-action="minimize" aria-label="Back to desktop">${icon('arrow')}</button><span id="window-title-${id}" class="os-window-title">${icon(APP_META[id][1])}${APP_META[id][0]}</span><div class="os-window-controls"><details class="window-arrangement"><summary aria-label="Arrange window" title="Arrange window">···</summary><div><button data-window-action="left">Tile left</button><button data-window-action="right">Tile right</button><button data-window-action="center">Center window</button></div></details><button data-window-action="minimize" aria-label="Minimize ${APP_META[id][0]}" title="Minimize">${icon('minus')}</button><button class="maximize-control" data-window-action="maximize" aria-label="Maximize ${APP_META[id][0]}" title="Maximize">${icon('expand')}</button><button data-window-action="close" aria-label="Close ${APP_META[id][0]}" title="Close">${icon('close')}</button></div></header><div class="app-content"></div><footer class="os-window-status"><span>hugo / ${id==='welcome'?'desktop':id}</span><span>${id==='terminal'?'⌘ / Ctrl K':id==='projects'?`${copy().projects.entries.length} ${copy().projects.items}`:'HUGO OS'}</span></footer><button class="window-resize" data-window-action="resize" aria-label="Resize ${APP_META[id][0]}" title="Drag to resize; arrow keys to adjust">⌟</button>`;
  const width=id==='welcome'?900:id==='terminal'?660:850;
  const height=id==='welcome'?600:id==='terminal'?410:640;
  const state={element,width,height,x:id==='welcome'?Math.max(120,(area.clientWidth-width)/2):140+(index%4)*36,y:id==='welcome'?24:28+(index%4)*30,closed:false,minimized:false,maximized:false,order:0,lastFocus:null};
...
function executePortfolioCommand(raw){const input=raw.trim();if(!input)return;const cmd=input.toLowerCase();commandHistory.push(input);historyIndex=commandHistory.length;if(cmd==='clear'){terminalHistory=[];openApp('terminal');renderApp('#terminal-command','terminal');return;}
 ...
 const navigation={about:'about',experience:'experience',projects:'projects',resume:'resume',contact:'contact'};
...
function introIsZh(){return window.HugoLocale?.language==='zh-TW';}
function introText(key,fallback){return window.HUGO_CONTENT?.[window.HugoLocale?.language]?.ui?.[key]||fallback;}
...
function autoEnterDesktop(){
  ...
  if(skipBtn)skipBtn.innerHTML=`${introIsZh()?'正在進入…':'Entering...'} <span aria-hidden="true">→</span>`;
}
...
async function startIntro(){
  ...
  $('#skip-intro').innerHTML=`${introText('skip','Skip intro')} <span aria-hidden="true">↗</span>`;
...
window.HugoOS?.init();
function syncLanguage({rerender=true}={}){
 const c=copy(),ui=c.ui;
 for(const [id,label] of Object.entries(ui.apps))if(APP_META[id])APP_META[id][0]=label;
 document.documentElement.lang=window.HugoLocale.language;document.title=c.seo.title;
 document.querySelector('meta[name="description"]')?.setAttribute('content',c.seo.description);
 $('#visitor-label').textContent=ui.visitor;
 $('#language-toggle').textContent=isZh()?'EN':'中';$('#language-toggle').setAttribute('aria-label',ui.switchLanguage);$('#language-toggle').title=ui.switchLanguage;
 $('.skip-link').textContent=isZh()?'跳到工作空間':'Skip to workspace';
 $('#intro-copy h1').innerHTML=isZh()?'每個系統，<br>都從某個地方開始。':'Every system starts<br>somewhere.';
 $('#intro-copy p').textContent=isZh()?'一個安靜的空間，一個連結世界的入口。':'A quiet room. A connected world.';
 $('#scroll-cue .scroll-text').textContent=ui.scroll;
 $('#skip-intro').innerHTML=`${ui.skip} <span aria-hidden="true">↗</span>`;$('#skip-intro').setAttribute('aria-label',ui.skip);
 $('#shortcut-welcome').textContent=ui.startHere;$('#shortcut-projects').textContent=ui.caseStudies;
 $('.desktop-shortcuts').setAttribute('aria-label',ui.shortcuts);$('.wallpaper-signature small').textContent=ui.signature;$('.desktop-caption').innerHTML=ui.caption.replaceAll('\n','<br>');
 $('#launcher-title').textContent=ui.applications;$('.launcher-heading small').textContent=ui.workspace;$('#shutdown-os').lastChild.textContent=` ${ui.shutdown}`;$('#shutdown-os').setAttribute('aria-label',ui.shutdown);
 $('#launcher').setAttribute('aria-label',ui.applications);$('#app-nav').setAttribute('aria-label',ui.allApps);$('.os-dock').setAttribute('aria-label',ui.taskbar);
 $('#launcher-toggle').setAttribute('aria-label',ui.applications);$('#launcher-toggle .dock-label').textContent=ui.apps;$('.dock-desktop').setAttribute('aria-label',ui.showDesktop);$('.dock-desktop .dock-label').textContent=ui.desktop;
 $('.system-settings').setAttribute('aria-label',ui.settings);$('.system-settings').title=ui.apps.settings;
 $('#app-nav').innerHTML=Object.entries(APP_META).filter(([id])=>id!=='welcome').map(([id,[label,ico]])=>`<button data-app="${id}">${icon(ico)}<span>${label}</span></button>`).join('');hydrateIcons($('#app-nav'));
 for(const [id,state] of windows){state.element.querySelector('.os-window-title').innerHTML=`${icon(APP_META[id][1])}${APP_META[id][0]}`;if(id==='projects')state.element.querySelector('.os-window-status span:last-child').textContent=`${c.projects.entries.length} ${c.projects.items}`;if(rerender&&!['browser','files','settings'].includes(id))renderApp(null,id);}
 syncDesktop();updateClock();
}
function updateClock(){$('#clock').textContent=new Intl.DateTimeFormat(isZh()?'zh-TW':'en-GB',{timeZone:'Asia/Taipei',hour:'2-digit',minute:'2-digit',hour12:window.HugoOS?.settings.hour12||false,...(window.HugoOS?.settings.seconds?{second:'2-digit'}:{})}).format(new Date());}
syncLanguage({rerender:false});hydrateIcons();syncDesktop();
window.addEventListener('hugo:language',()=>syncLanguage());
...
document.addEventListener('click',event=>{
 ...
 if(button.dataset.yearLaunch){selectedYear=button.dataset.yearLaunch;openApp('experience');renderApp(`[data-year="${selectedYear}"]`,'experience');return;}
 if(button.dataset.projectLaunch!==undefined){selectedProject=Number(button.dataset.projectLaunch);projectDetail=false;openApp('projects');renderApp(`[data-select-project="${selectedProject}"]`,'projects');return;}
 if(button.dataset.app){openApp(button.dataset.app);return;}
 if(button.dataset.action==='language'){window.HugoLocale.set(isZh()?'en':'zh-TW');return;}
 ...
 if(button.dataset.projectFilter){projectFilter=button.dataset.projectFilter;const projects=copy().projects.entries,current=projects[selectedProject],valid=projectFilter==='all'||(projectFilter==='backend'?current.kind!=='portfolio':current.kind==='portfolio');if(!valid)selectedProject=projects.findIndex(project=>projectFilter==='portfolio'?project.kind==='portfolio':project.kind!=='portfolio');renderApp(`[data-project-filter="${projectFilter}"]`,'projects');return;}
...
$('#shutdown-os').addEventListener('click',startIntro);
...
window.addEventListener('hashchange',()=>{
 const requested=location.hash.slice(1),id=['architecture','database'].includes(requested)?'projects':requested;
 if(requested!==id)history.replaceState(null,'',location.pathname+location.search+'#projects');
...
const requestedInitialApp=location.hash.slice(1),initialApp=['architecture','database'].includes(requestedInitialApp)?'projects':requestedInitialApp;
if(requestedInitialApp!==initialApp)history.replaceState(null,'',location.pathname+location.search+'#projects');
